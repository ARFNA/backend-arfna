...
public class ImageIdApiV1 implements IImageIdApi {
    ...
    @Override
    public ImageIdResponse getResponse(ImageIdPayload payload, EVersion version, Optional<Subscriber> subscriber) {
        ...
        if (requestType == EImageRequest.GENERATE_ID) {
            ...
            return helper.generateImageId(payload, version, subscriber.get());
        } if (requestType == EImageRequest.UPLOAD_IMAGE) {
            ImageIdHelper helper = new ImageIdHelper();
            ImageIdResponse response = checkPermissions(payload, version, subscriber, helper);
            if (response != null) return response;
            ArfnaLogger.debug(this.getClass(), "Uploading thumbnail");
            return helper.uploadImage(payload, version, subscriber.get());
    ...
    private ImageIdResponse checkPermissions(ImageIdPayload payload, EVersion version, Optional<Subscriber> subscriber, ImageIdHelper helper) {
